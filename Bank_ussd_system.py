...
def set_transaction_pin():
    ...
    """
    while True:
        pin = input("Kindly set your Transaction PIN (4 digits): ")
        if pin.isdigit() and len(pin) == 4:
            return pin
        print("Invalid PIN. Must be 4 digits.")


def transfer_to_moniepoint(balance, pin):
    """
    ...
    total = amount
    return process_transaction(balance, pin, amount, total, f"Transferring N{amount}.00 to {account}.")


def transfer_to_other_banks(balance, pin):
    """
    ...
    fee = 20
    total = amount + fee
    return process_transaction(balance, pin, amount, total, f"Transferring N{amount}.00 to {account} ({bank}).\nFee: N{fee}")
...
def process_transaction(balance, pin, amount, total, message):
    """
    ...
    if entered_pin == pin:
        if total > balance:
            ...
            balance -= total
            print(f"Transaction successful. Updated balance: N{balance}.00")
    else:
        print("Incorrect PIN!")
    return balance
...
def buy_airtime(balance, pin, user_number, line):
    """
    Allows user to buy airtime for themselves or others.
    Applies a discount and processes transaction.
    Returns updated balance.
    """
    recipient, amount = select_airtime_recipient(user_number, line)
    discount = 10
    total = amount - discount
    return process_transaction(balance, pin, amount, total, f"Buying N{amount}.00 {line} airtime for {recipient}. Discount: N{discount}")


def select_airtime_recipient(user_number, line):
    """
...
def buy_data(balance, pin, user_number, line):
    """
    ...
    data_plans = {
        "MTN": [["1.8GB - N1500", "2.7GB - N2000"], ["750MB - N500", "1.5GB - N1000"], ["100MB - N100", "200MB - N200"]],
        ...
        "9MOBILE": [["2GB - N1500", "3.5GB - N2000"], ["800MB - N500", "1.5GB - N1000"], ["120MB - N100", "250MB - N200"]]
    }

    selected_plan = select_from_list(f"Select {network} {data_type[plan_group]} plan:", data_plans[network][plan_group])
    price = int(data_plans[network][plan_group][selected_plan].split(" - N")[-1].replace(",", ""))
    return process_transaction(balance, pin, price, price, f"Buying {data_plans[network][plan_group][selected_plan]} for {recipient} ({network})")
...
def check_balance(balance, pin):
    ...
    if input("Enter PIN to check balance:\n") == pin:
        print(f"Your balance is N{balance}.00")
    else:
        print("Invalid PIN!")
    return balance
